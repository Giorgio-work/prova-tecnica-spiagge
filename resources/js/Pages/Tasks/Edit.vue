  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="container mx-auto p-4">
      <div class="flex items-center mb-6">
        ...
        <h1 class="text-2xl font-bold">Modifica Task</h1>
      </div>
      ...
      <div class="bg-white rounded-lg shadow p-6">
        <form @submit.prevent="submitForm" class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div v-if="task.completed_at" class="flex items-center">
              <div class="text-sm">
                <span class="font-medium">Completato il:</span>
                <span class="ml-2">{{ formatDate(task.completed_at) }}</span>
              </div>
            </div>
          </div>
...
const form = useForm({
  ...
  due_date: props.task.due_date?.slice(0, 16) || '',
})
...
const breadcrumbs = computed(() => [
  ...
  { title: 'Modifica', href: `/tasks/${props.task.id}/edit` },
])
